...
|--------|-----------|---------------------|
| **Factuality/Accuracy (Độ chính xác dữ liệu)** | > 95% (So với Ground Truth) | < 90% (Dừng deploy, quay lại chỉnh RAG) |
| **Human Fallback Rate (Tỷ lệ chuyển tư vấn viên)** | < 30% (AI tự xử lý được >70%) | > 50% (AI vô dụng, làm phiền user thêm) |
| **P90 Latency (Thời gian phản hồi)** | < 10s | > 12s (User mất kiên nhẫn, bỏ đi) |
...
*   **Hồ Nhất Khoa (Data & Pipeline Engineer):** Xây dựng RAG pipeline. Viết script crawl/xử lý data review từ các hội nhóm, thực hiện Document Chunking, Embedding và lưu vào Vector DB. Tích hợp API gọi LLM với độ trễ (latency) tối ưu < 10s.

*   **Đặng Tùng Anh (Risk & Eval Lead):** Định nghĩa và setup framework đo lường. Tập trung test các Failure Modes (đặc biệt là nhầm lẫn xe VF6/VF8 và lỗi giá tiền). Đảm bảo chỉ số Factuality > 95% trước khi demo. Định đoạt ngưỡng "Kill criteria".
